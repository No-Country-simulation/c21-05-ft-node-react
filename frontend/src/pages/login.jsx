...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      login(response.data); // Guarda datos en el context
      ...
      console.error('Error en el login:', axiosError);
    } finally {
      setIsLoading(false);
    }
  }
      ...
      <div className="max-w-md min-h-lvh mx-auto my-8 p-6 rounded-lg text-left ">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          >
            ...
            <span className={`${isLoading ? 'text-transparent' : ''}`}>Iniciar Sesión</span>
          </button>
        </form>
        ...
        <div className='mt-4 flex-row items-center text-right'>
          <Link to="/signup" className="h-6 w-6 hover:text-yellow-300 ">
            <ArrowRightIcon className='w-6 inline text-yellow-400' />
            <span>No tengo cuenta, quiero registrarme.</span>
          </Link>
        </div>
      </div>
    </Layout>
